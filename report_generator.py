...
# Research Landscape: {domain} — Last {days} Days
*Generated: {date}*
...
def retrieve_all_papers_from_db() -> list[dict]:
    ...
    try:
        ...
        for doc in results:
            ...
            if full:
                papers.append(full)
        print(f"[Report] 从数据库取出 {len(papers)} 篇论文")
        return papers
    except Exception as e:
        print(f"[Report] 数据库读取失败：{e}")
        return []
...
def generate_report(
    ...
    """
    if not papers:
        return "错误：没有论文数据，请先运行 agent.py。"
...
def clean_markdown_tables(text: str) -> str:
    """
    修复 Gemini 生成的 Markdown 表格问题：
    1. 将过长的分隔行 | :---...--- | 规范化为 | --- |
    2. 去掉纯空白的超长行（LLM 有时会输出数十万空格代替表格行）
...
def should_render_section(content) -> bool:
    """
    内容门控：返回 False 表示该章节应完全隐藏。
    content 可以是 None / 空列表 / 字符串。
    """
    if content is None:
        return False
...
def strip_empty_sections(report_text: str) -> str:
    """
    对 Gemini 生成的报告进行后处理：
    移除内容中包含 EMPTY_SECTION_PHRASES 的 Section 4（Gaps）块。
    将文档按 ## 级标题拆分，对 ## 4. 进行门控，再重新拼接。
    """
    import re
...
def enhance_reading_order(report_text: str, papers: list[dict]) -> str:
    ...
    策略：
    1. 提取 "## 5." 章节内容
    2. 按行扫描，识别以数字+点或 ** 开头的条目标题
    3. 在每个条目标题行下插入链接 + BibTeX（通过标题模糊匹配论文）
    4. 替换回原文档
    """
    import re
...
def render_methodology_matrix(papers: list[dict], llm=None) -> str:
    ...
    for p in papers:
        ...
        approach = mm.get("approach_type", "—")
        arch = mm.get("model_architecture") or "—"
        supervision = mm.get("supervision_type", "—")
        modality = ", ".join(mm.get("data_modality", [])) or "—"
        ...
        if oss == "yes" and github_url_validated and github_url:
            open_source_display = f"✅ [Code]({github_url})"
        elif oss == "yes":
            open_source_display = "✅ Available (link unverified)"
        elif oss == "no":
            open_source_display = "❌ Not available"
        else:
            open_source_display = "—"
        theory = mm.get("theoretical_guarantees", "—")
    ...
    if has_missing:
        lines.append(
            "> ℹ️ Some papers lack methodology matrix data. "
...
if __name__ == "__main__":
    ...
    if not papers:
        print("\n❌ 数据库为空，请先运行：python agent.py")
    else:
        # Step 2：可信度评分 + 置信度判断
        print("[Report] 正在进行可信度评分...")
